...
from att.ECA import ECAAttention
from att.CBAM import CBAMBlock
from att.SE import SEAttention
##### basic ####
...
class Conv_attp(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Conv_attp, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.att = SEAttention(c1)

    def forward(self, x):
        return self.act(self.bn(self.conv(self.att(x))))

    def fuseforward(self, x):
        return self.act(self.conv(self.att(x)))
...
class PConvde_att(nn.Module):
    ...
    def forward_split_cat(self, x):
        ...
        x = torch.cat((x1, x2), 1)
        x = self.conv(x)
        return self.att(self.act(self.bn(x)))

class PConvlink50(nn.Module):
...
class PConvlink75(nn.Module):
    def __init__(self, dim, ouc, n_div=4, forward='split_cat'):
        super().__init__()
        self.dim_conv3 = (dim // n_div)*3
        self.dim_untouched = dim - self.dim_conv3
        self.partial_conv3 = nn.Conv2d(self.dim_conv3, self.dim_conv3, 3, 1, 1, bias=False)
        self.conv = Conv(dim, ouc, k=1)
        self.bn = nn.BatchNorm2d(ouc)
